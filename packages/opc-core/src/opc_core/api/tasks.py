...
async def start_task(
    ...
    if task.status not in [TaskStatus.ASSIGNED.value, TaskStatus.PENDING.value]:
        raise HTTPException(
            status_code=400, detail=f"Task status is {task.status}, cannot start"
        )
...
"""
    ...
    async with Messenger() as messenger:
        response = await messenger.send(
            agent_id=employee.openclaw_agent_id, message=message, timeout=300
        )
...
async def send_message(
    ...
    async with Messenger() as messenger:
        response = await messenger.send(
            agent_id=employee.openclaw_agent_id, message=data.content, timeout=300
        )
