...
class AgentLog(Base):
    ...
    # 核心内容 - 原始文本（限制在合理范围内）
    content: Mapped[str] = mapped_column(Text, nullable=False)  # 发送/接收的内容
    response: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # 如果是outgoing，这里存储Agent的回复

    # 关联信息
    task_id: Mapped[Optional[str]] = mapped_column(String(32), nullable=True, index=True)
    meta_info: Mapped[Optional[Dict[str, Any]]] = mapped_column(JSON, nullable=True, default=dict)

    # 时间戳（覆盖 Base 的 created_at 以添加 index）
