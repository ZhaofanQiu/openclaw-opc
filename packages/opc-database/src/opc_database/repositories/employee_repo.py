...
class EmployeeRepository(BaseRepository[Employee]):
    ...
    async def get_available_for_task(
        self, estimated_cost: float = 0.0, limit: int = 10
    ) -> List[Employee]:
        """
        获取可接受任务的员工列表
    ...
    async def update_budget(
        self, employee_id: str, amount: float, operation: str = "add"
    ) -> Optional[Employee]:
        """
        更新员工预算
    ...
    async def bind_openclaw_agent(
        self, employee_id: str, openclaw_agent_id: str
    ) -> Optional[Employee]:
        """
        绑定OpenClaw Agent
    ...
    async def get_budget_stats(self) -> dict:
        ...
        result = await self.session.execute(
            select(
                ...
                func.avg(Employee.monthly_budget - Employee.used_budget).label(
                    "avg_remaining"
                ),
            )
        )
