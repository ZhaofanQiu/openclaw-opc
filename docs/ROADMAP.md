...
**像素办公室 V2 (重点)**:
- [ ] **个性化头像系统** - Partner为每个员工创作独特头像
  - 头像生成流程：Partner根据员工特点生成描述 → Vivago AI生成/Partner编写SVG
  - 存储到 avatar_svg 字段
  - 同一职业不同员工有差异（如两个前端：一个戴眼镜、一个戴耳机）
  - 头像可随状态变化（工作中出汗、完成项目后升级造型）
- [ ] **头像清晰度提升** - 12x12 → 24x24或32x32像素
- [ ] **更多个性化定制** - 服装颜色、配饰、工位装饰等
...
| 像素头像 | 12x12 SVG 职业模板 | 24x32+ 个性化生成 | 低 (V2) |
