 ...
 * Returns true if auth is required, false if public access
 */
...
async function checkAuthRequired() {
    ...
    try {
        ...
        // If 401, auth is required
        authRequiredCache = response.status === 401;
        return authRequiredCache;
    } catch (error) {
        console.error('Auth check error:', error);
        // Default to requiring auth on error
...
async function authFetch(url, options = {}) {
    ...
    // If auth is required and no API key, show login
    ...
    if (apiKey) {
        authOptions.headers['Authorization'] = `Bearer ${apiKey}`;
    }
